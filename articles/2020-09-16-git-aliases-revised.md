...
`peco`を利用するaliasは引数があれば短縮Wordとして、そうではないなら、`peco`での選択になるようにしてある。
`fzf`にも対応した。（Windowsでwinptyなしのインタラクティブ動作が安定したこともあり）
あと、その選択もローカルのブランチだけとか追跡ブランチも込みとか、ログからのハッシュ選択とかある。

改良が必要な箇所としては、`elif`にしてあるのは`else`でもよいんじゃないか（`peco`がないなら常に短縮動作）とか考えてますが、まだ未着手。
...
管理にこの記事での [gist](https://gist.github.com/tsuyoshicho/0a8610e87f209f5e86bc) を利用するって手はあるけど、ファイルメンテナンスをローカル・gistではできなくて、記事自体で書き換えが必要になるのはちょっとつらいな...。
