...
Vimは補完として基本の補完やomni補完、他に類語辞書での補完などが手動で呼べる機能があります。
...
この経験と、あと`asyncomplete.vim`の補完ソース複数[^4][^5][^6]についてcontributeした経験がありました。
...
もっとも、`asyncomplete.vim`の補完ソースはプラグインとして `/autoload/xxx.vim` や `/plugin/xxx.vim` は不要なので、後で削除しましたが……。
...
さて、最初にmrr(git リポジトリのリスト)で作成を行いましたが、全部を1つのソースとして作るとコントロールが効かなくなって嬉しくありません。
なので、mrwそしてmruについて同じファイルをコピーしてそれぞれをソースとして持つようにしました。
...
この辞書データのリストに対応するための修正を行いました。

Vimの辞書は他の言語でいうところの連想配列みたいなものです(Pythonのdictというべきか)。
`mr.vim`は単純な文字列リストを返していましたし、これを直に設定していましたが、これに構造を持せます。
辞書構造に変換して、ソースが何なのか、他プロパティをいくつかもたせるようにしました。

#### マッチルール

`asyncomplete.vim`では補完ソースが呼ばれるとき、補完時の文字を自前で確認することができます。
ここで不一致なら空で終了することで高速化されているわけです。

この処理の所を、初期は `\w+` としていたのですが、`mr.vim`が扱うのはパスですので、これはあまり適切ではありませんでした。
ここを `\f*` としました(1文字は必須でもよかったかも)。
こちらはVimの `isfname` というオプション依存ながら、パスに使う文字列が適合するので、より適切です。
これにより `:` や `/` や `\` でもマッチします。

ただし、補完のトリガーや終了はVimの `iskeyword` というオプションに依存しています。
