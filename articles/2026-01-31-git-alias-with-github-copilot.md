...
それもあわせて、自分の `git` の alias に worktree 系の便利なの用意したいな、というがありました。
...
やり方で対応はできると思うのですが、作成対象も大きい予定ではなかったので、泥縄で対応し続けて完了まで持って行きました。
...
位置は "<repo name>/<branch name>/" つまり "../<branch name>/" に作るようにしています。
...
また、残るブランチでのトラブル回避のため、リモート追跡ブランチもあわせて削除します。
