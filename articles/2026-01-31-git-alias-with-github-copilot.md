...
1. それで慣れたので、自分のローカルでも使ってみた(GitHub の Free版)
1. 感想と結果
...
- `git wtc` : `git clone` するが、worktree 展開に便利なフォルダ構成で行う
  - `<repo>/<default-branch>.git` が最初だった
  - 今は調整して `<repo>/<repo>.git` に no-checkout で cloneし、`<repo>/<default-branch>` という worktree を初期作成する
